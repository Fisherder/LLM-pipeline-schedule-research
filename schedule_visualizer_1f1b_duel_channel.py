# 文件名: schedule_visualizer_dual_channel.py
# 描述: 这是一个为了对比可视化方案而创建的版本。
#       它的核心计算逻辑与主版本完全相同（所有通信任务在单一通道上竞争资源），
#       但绘图逻辑被修改，将不同方向的通信任务分别绘制在独立的“上游”和“下游”通道中，以更清晰地展示数据流向。
# V9更新: 实现“主动独占”强优先级逻辑。
...
default_priorities = {
    ...
    'fwd_prop': 0.3, 'grad_prop': 0, 
    'grad_sync': 0.2, 'param_sync': 1
}
# [MODIFIED] 定义主动独占任务及其等级 ('a' > 'b' > 'c'...)
# None 表示该任务为普通共享任务
default_exclusive_tiers = {
    'fwd_prop': 'a',   # 'a', 'b', 'c'... or None
...
def plot_single_gpu_chart(gpu_rank, all_blocks, total_time, throttled_task_ids, show_throttling, show_plot):
    ...
    print(f"\n为 GPU {gpu_rank+1} 生成的双通道可视化图表已保存为: {output_filename}")
...
def plot_total_pipeline_chart(all_blocks, total_time, num_gpus, throttled_task_ids, show_throttling, show_completion_time, show_plot):
    ...
    output_filename = 'pipeline_schedule_total_dual_channel_viz.png'
    plt.savefig(output_filename, dpi=150)
