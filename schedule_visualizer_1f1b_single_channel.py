...
def plot_single_gpu_chart(gpu_rank, all_blocks, total_time, throttled_task_ids, show_throttling, show_plot):
    ...
    fig.suptitle(f"GPU {gpu_rank + 1} - 1F1B Schedule (Single Comm Visualization)", fontsize=20, y=0.98)
...
def draw_polygon_for_segment(ax, segment, throttled_task_ids, show_throttling):
    ...
    ax.text(min_start + (max_end - min_start) / 2, avg_y_center, segment[0]['label'], ha='center', va='center', color='black', fontsize=16, weight='bold', zorder=2)
...
def plot_total_pipeline_chart(all_blocks, total_time, num_gpus, throttled_task_ids, show_throttling, show_completion_time, show_plot):
    fig, axes = plt.subplots(num_gpus, 1, figsize=(24, 3.0 * num_gpus), sharex=True, gridspec_kw={'hspace': 0})
    fig.suptitle("Total 1F1B Pipeline Schedule (Single Comm Visualization)", fontsize=28, y=0.95)
    ...
    for gpu_rank in range(num_gpus):
        ...
        ax.set_ylabel(f"GPU {gpu_rank + 1}", fontsize=20, rotation=0, labelpad=44, va='center')
        ax.set_ylim(0, num_channels)
        ax.set_yticks([i + 0.5 for i in range(num_channels)])
        ax.set_yticklabels(visual_layout_labels, fontsize=16)
        ax.grid(axis='x', linestyle='--', alpha=0.7)
    ...
    plt.xticks(np.arange(0, int(total_time * 1.05) + 2, 2), fontsize=18)
    ...
    legend_patches = [mpatches.Patch(color=c, label=t.replace('_', ' ').title()) for t, c in colors.items()]
    fig.legend(handles=legend_patches, loc='lower center', bbox_to_anchor=(0.5, 0.02), ncol=len(colors), fontsize=20)
